...
class BoxDescriber(nn.Module):
    ...
    def forward_test(self, feat):
        ...
        # Ensure keep is on the same device as predicts
        keep = torch.arange(batch_size,).to(device)  # keep track of unfinished sequences
        ...
        for i in range(self.max_len):
            ...
            # Need to handle the case where keep becomes empty
            if keep.nelement() == 0:
                break

            # Ensure h and c are on the correct device and have the correct shape for the current batch size
            current_h = h[:, keep, :] if h.shape[1] == batch_size else h
            current_c = c[:, keep, :] if c.shape[1] == batch_size else c


            rnn_input = word_emb.unsqueeze(1)
            # Ensure rnn_input and hidden states are on the same device
            rnn_input = rnn_input.to(device)
            current_h = current_h.to(device)
            current_c = current_c.to(device)


            rnn_output_valid, (h, c) = self.rnn(rnn_input, (current_h, current_c))  # (num_layers, valid_batch_size, hidden_size)

            if self.fusion_type == 'init_inject':
                rnn_output = h[-1]
            else: # merge
                # Ensure feat_emb[keep] is on the same device
                rnn_output = torch.cat([h[-1], feat_emb[keep].to(device)], dim=-1)  # (valid_batch_size, hidden_size + emb_size)

            pred = self.fc_layer(rnn_output)  # (valid_batch_size, vocab_size)

            # Ensure predicts[keep, i+1] is on the same device as pred
            predicts[keep, i+1] = pred.log_softmax(dim=-1).argmax(dim=-1).to(device)
            ...
            if keep.nelement() == 0:  # stop if all finished
                ...
                # These slices should already be on the correct device if keep is on the correct device
                h = h[:, non_stop, :]
                c = c[:, non_stop, :]
