...
class DenseCapDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(self.vg_data_path, 'r') as vg_data:
            ...
            img_path = os.path.join(self.img_dir_root, self.look_up_tables['idx_to_directory'][vg_idx].lstrip('images/'),
...
class DenseCapDatasetV2(Dataset):
    ...
    def __getitem__(self, idx):
        # Open the HDF5 and get VG index
        with h5py.File(self.vg_data_path, 'r') as vg_data:
            vg_idx = (self.look_up_tables['split'][self.dataset_type][idx]
                      if self.dataset_type else idx)

            # Build the correct image path (no 'images/' subfolder)
            filename = self.look_up_tables['idx_to_filename'][vg_idx]
            img_path = os.path.join(self.img_dir_root, filename)
            ...
            # Load boxes & captions
            first = vg_data['img_to_first_box'][vg_idx]
            last  = vg_data['img_to_last_box'][vg_idx]
            boxes    = torch.as_tensor(vg_data['boxes'][first:last+1],    dtype=torch.float32)
            caps     = torch.as_tensor(vg_data['captions'][first:last+1],dtype=torch.long)
            caps_len = torch.as_tensor(vg_data['lengths'][first:last+1], dtype=torch.long)

        targets = {'boxes': boxes, 'caps': caps, 'caps_len': caps_len}
        info    = {'idx': vg_idx,      'file_name': filename}
        return img, targets, info
